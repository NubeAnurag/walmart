...
import { Truck, Eye, EyeOff, Loader2, Mail, Lock, ArrowLeft, User } from 'lucide-react';
...
const SupplierLoginPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const result = await login({ ...formData, role: 'supplier' });
      
      if (result.success) {
        if (result.user.role === 'supplier') {
          navigate('/dashboard/supplier');
        } else {
          setErrors({ general: 'Access denied. Supplier account required.' });
        }
      }
    } catch (error) {
